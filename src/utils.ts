...
export const isFinishedTournamentActual = (tournament: Tournament, rounds: Round[]): boolean => {
  const actualRoundsPlayed = calculateActualRoundsPlayed(rounds);
  return actualRoundsPlayed === tournament.total_rounds;
};

export const isOngoingTournamentActual = (tournament: Tournament, rounds: Round[]): boolean => {
  const actualRoundsPlayed = calculateActualRoundsPlayed(rounds);
  return actualRoundsPlayed > 0 && actualRoundsPlayed < tournament.total_rounds;
};

export const isDraftTournamentActual = (_tournament: Tournament, rounds: Round[]): boolean => {
  const actualRoundsPlayed = calculateActualRoundsPlayed(rounds);
  return actualRoundsPlayed === 0;
};

export const getTournamentStatusActual = (tournament: Tournament, rounds: Round[]): 'draft' | 'ongoing' | 'finished' => {
...
export const getTournamentProgressActual = (tournament: Tournament, rounds: Round[]): number => {
  const actualRoundsPlayed = calculateActualRoundsPlayed(rounds);
  return tournament.total_rounds > 0 ? (actualRoundsPlayed / tournament.total_rounds) * 100 : 0;
};
