...
export const exportStandingsToCsv = (standings: PlayerStanding[], tournamentName: string) => {
  // CSV headers
  const headers = [
    'Rank',
    'Name',
  ...
  link.setAttribute('download', `${tournamentName.replace(/\s+/g, '_')}_standings.csv`);
...
export const exportStandingsToPdf = async (_standings: PlayerStanding[], _tournamentName: string) => {
