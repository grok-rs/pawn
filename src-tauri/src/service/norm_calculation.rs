...
impl<D: Db> NormCalculationService<D> {
    ...
    fn group_players_by_rank<'a>(
        &self,
        standings: &'a [crate::domain::tiebreak::PlayerStanding],
    ) -> HashMap<i32, Vec<&'a crate::domain::tiebreak::PlayerStanding>> {
    ...
    fn distribute_prize_for_tied_players(
        &self,
        prize_amount: f64,
        players: &[&crate::domain::tiebreak::PlayerStanding],
    ...
    async fn get_tournament_config(
        &self,
        tournament_id: i32,
    ) -> Result<crate::domain::tiebreak::TournamentTiebreakConfig, PawnError> {
        match self.db.get_tournament_settings(tournament_id).await? {
            Some(config) => Ok(config),
            None => Ok(crate::domain::tiebreak::TournamentTiebreakConfig {
                tournament_id,
                ..Default::default()
            }),
        }
