...
use crate::{
    ...
    standings::service::tiebreak::TiebreakCalculator,
};
...
impl<D: Db> NormCalculationService<D> {
    ...
    async fn generate_additional_info(
        ...
        if requirements.performance_rating_met {
            info.push("V Performance rating requirement met".to_string());
        } else {
            info.push("X Performance rating requirement not met".to_string());
        }

        if requirements.minimum_games_met {
            info.push("V Minimum games requirement met".to_string());
        } else {
            info.push("X Minimum games requirement not met".to_string());
        }

        if requirements.minimum_score_met {
            info.push("V Minimum score requirement met".to_string());
        } else {
            info.push("X Minimum score requirement not met".to_string());
        }
    ...
    fn group_players_by_rank<'a>(
        &self,
        standings: &'a [crate::standings::model::PlayerStanding],
    ) -> HashMap<i32, Vec<&'a crate::standings::model::PlayerStanding>> {
    ...
    fn distribute_prize_for_tied_players(
        &self,
        prize_amount: f64,
        players: &[&crate::standings::model::PlayerStanding],
    ...
    async fn get_tournament_config(
        &self,
        tournament_id: i32,
    ) -> Result<crate::standings::model::TournamentTiebreakConfig, PawnError> {
        match self.db.get_tournament_settings(tournament_id).await? {
            Some(config) => Ok(config),
            None => Ok(crate::standings::model::TournamentTiebreakConfig {
                tournament_id,
                ..Default::default()
            }),
        }
