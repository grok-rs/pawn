...
use crate::{
    ...
    db::*,
    export::model::{ExportFormat, ExportRequest, ExportResult, ExportType},
    participant::model::Player,
    standings::model::{CrossTable, StandingsCalculationResult, TournamentTiebreakConfig},
...
impl<D: Db> ExportService<D> {
    ...
    fn get_html_styles(&self, request: &ExportRequest) -> &'static str {
        ...
        match request.template_options.as_ref().map(|o| &o.color_scheme) {
            Some(crate::export::model::ColorScheme::Professional) => {
                include_str!("../../templates/professional.css")
            }
            Some(crate::export::model::ColorScheme::Minimal) => {
                include_str!("../../templates/minimal.css")
            }
            Some(crate::export::model::ColorScheme::Classic) => {
