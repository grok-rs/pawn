...
use crate::{
    ...
    team::pairing_service::{
        BoardOrderPolicy, ColorAllocation, TeamPairingConfig, TeamPairingEngine, TeamPairingMethod,
    },
    team::scoring_service::{
        TeamScoringConfig, TeamScoringService, TeamScoringSystem, TeamTiebreakCriterion,
    },
    team::service::TeamStatistics,
};
...
pub struct TeamPairingResultDto {
    ...
    pub individual_pairings: Vec<crate::competition::model::Pairing>,
...
pub async fn calculate_team_standings(
    ...
    let board_weight_system = match config.board_weight_system.as_str() {
        "equal" => crate::team::scoring_service::BoardWeightSystem::Equal,
        "descending" => crate::team::scoring_service::BoardWeightSystem::Descending,
        "ascending" => crate::team::scoring_service::BoardWeightSystem::Ascending,
        _ => {
            return Err(crate::common::error::PawnError::InvalidInput(
    ...
    for criterion_str in &config.tiebreak_criteria {
        let criterion = match criterion_str.as_str() {
            ...
            _ => {
                return Err(crate::common::error::PawnError::InvalidInput(format!(
                    "Invalid tiebreak criterion: {criterion_str}"
                )));
            }
        };
...
pub async fn validate_team_scoring_config(
    ...
    for criterion in &config.tiebreak_criteria {
        match criterion.as_str() {
            ...
            _ => {
                return Err(crate::common::error::PawnError::InvalidInput(format!(
                    "Invalid tiebreak criterion: {criterion}"
                )));
            }
        }
    }
