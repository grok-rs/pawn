...
use crate::pawn::{
    ...
    db::Db,
    domain::{
        dto::{CreateRound, UpdateRoundStatus, GeneratePairingsRequest, CreateGame},
        model::{Round, RoundDetails, RoundStatus, PairingMethod, Pairing, GameResult},
    },
...
impl<D: Db> RoundService<D> {
    ...
    pub async fn get_rounds_by_tournament(&self, tournament_id: i32) -> Result<Vec<Round>, PawnError> {
        self.db
    ...
    pub async fn create_round(&self, data: CreateRound) -> Result<Round, PawnError> {
        ...
        let _tournament = self.db.get_tournament(data.tournament_id).await
            .map_err(PawnError::Database)?;
        ...
        if let Ok(existing_rounds) = self.db.get_rounds_by_tournament(data.tournament_id).await {
            if existing_rounds.iter().any(|r| r.round_number == data.round_number) {
    ...
    pub async fn get_round_details(&self, round_id: i32) -> Result<RoundDetails, PawnError> {
        let round = self.db.get_round(round_id).await.map_err(PawnError::Database)?;
        let games = self.db.get_games_by_round(round.tournament_id, round.round_number).await
            .map_err(PawnError::Database)?;
    ...
    pub async fn generate_pairings(&self, request: GeneratePairingsRequest) -> Result<Vec<Pairing>, PawnError> {
        tracing::info!("Generating pairings for tournament {}, round {}, method: {}", 
                      request.tournament_id, request.round_number, request.pairing_method);
        
        // Get tournament and validate pairing method
        let tournament = self.db.get_tournament(request.tournament_id).await
            .map_err(PawnError::Database)?;
        
        tracing::debug!("Found tournament: {} with {} total rounds", tournament.name, tournament.total_rounds);
        ...
        if players.is_empty() {
            ...
            return Err(PawnError::InvalidInput("No players found for tournament".into()));
        }

        // Get current player results for Swiss pairing
        let player_results = self.db.get_player_results(request.tournament_id).await
            .map_err(PawnError::Database)?;
        ...
        let pairings = if pairing_method == PairingMethod::Swiss && request.round_number > 1 {
            ...
            let all_games = self.db.get_games_by_tournament(request.tournament_id).await
                .map_err(PawnError::Database)?;
                
            // Create a lookup map for players
            let player_map: std::collections::HashMap<i32, &crate::pawn::domain::model::Player> = players
                .iter()
                .map(|p| (p.id, p))
                .collect();
            ...
            tracing::debug!("Found {} games in history for pairing analysis", game_history.len());
    ...
    pub async fn create_pairings_as_games(&self, tournament_id: i32, round_number: i32, pairings: Vec<Pairing>) -> Result<Vec<GameResult>, PawnError> {
        // Check if games already exist for this round
        let existing_games = self.db.get_games_by_round(tournament_id, round_number).await
            .map_err(PawnError::Database)?;
    ...
    pub async fn create_next_round(&self, tournament_id: i32) -> Result<Round, PawnError> {
        ...
        let rounds = self.db.get_rounds_by_tournament(tournament_id).await
            .map_err(PawnError::Database)?;
        ...
        let tournament = self.db.get_tournament(tournament_id).await
            .map_err(PawnError::Database)?;
