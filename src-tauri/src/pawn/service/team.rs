...
use crate::pawn::domain::dto::{
    CreateTeam, UpdateTeam, AddPlayerToTeam, RemovePlayerFromTeam, CreateTeamMatch, 
    UpdateTeamMatch, CreateTeamLineup, CreateTeamTournamentSettings, UpdateTeamTournamentSettings,
    CreateTeamBoardRules, TeamSearchFilters
};
use crate::pawn::domain::model::{
    Team, TeamMembership, TeamMatch, TeamLineup, TeamTournamentSettings, TeamBoardRules, 
    Player, Tournament, TeamStanding
};
use std::sync::Arc;
...
impl<D: Db> TeamService<D> {
    ...
    pub async fn create_team(&self, data: CreateTeam) -> Result<Team, PawnError> {
        ...
        if self.is_team_name_taken(data.tournament_id, &data.name).await? {
            return Err(PawnError::ValidationError(
                format!("Team name '{}' is already taken in this tournament", data.name)
            ));
        }
        
        // Verify tournament exists and is a team tournament
        self.validate_tournament_for_teams(data.tournament_id).await?;
    ...
    pub async fn update_team(&self, data: UpdateTeam) -> Result<Team, PawnError> {
        ...
        if data.name.is_none() && data.captain.is_none() && data.description.is_none() 
            && data.color.is_none() && data.club_affiliation.is_none() 
            && data.contact_email.is_none() && data.contact_phone.is_none() 
            && data.max_board_count.is_none() && data.status.is_none() {
            return Err(PawnError::ValidationError(
        ...
        if let Some(ref name) = data.name {
            ...
            if let Ok(existing_team) = self.db.get_team_by_id(data.id).await {
                if existing_team.name != *name {
                    if self.is_team_name_taken(existing_team.tournament_id, name).await? {
                        return Err(PawnError::ValidationError(
                            format!("Team name '{}' is already taken in this tournament", name)
                        ));
                    }
                }
            }
    ...
    pub async fn delete_team(&self, team_id: i32) -> Result<(), PawnError> {
        ...
        let team = self.db.get_team_by_id(team_id).await.map_err(PawnError::from)?;
        ...
        self.db.delete_team(team_id).await.map_err(PawnError::from)?;
    ...
    pub async fn get_team_by_id(&self, team_id: i32) -> Result<Team, PawnError> {
        self.db.get_team_by_id(team_id).await.map_err(PawnError::from)
    }

    /// Get all teams for a tournament
    #[instrument(skip(self))]
    pub async fn get_teams_by_tournament(&self, tournament_id: i32) -> Result<Vec<Team>, PawnError> {
        self.db.get_teams_by_tournament(tournament_id).await.map_err(PawnError::from)
    }
    ...
    #[instrument(skip(self))]
    pub async fn add_player_to_team(&self, data: AddPlayerToTeam) -> Result<TeamMembership, PawnError> {
        info!("Adding player {} to team {} on board {}", data.player_id, data.team_id, data.board_number);
        
        // Validate team and player exist
        let team = self.db.get_team_by_id(data.team_id).await.map_err(PawnError::from)?;
        let player = self.db.get_player_by_id(data.player_id).await.map_err(PawnError::from)?;
        ...
        if data.board_number < 1 || data.board_number > team.max_board_count {
            return Err(PawnError::ValidationError(
                format!("Board number must be between 1 and {}", team.max_board_count)
            ));
        }
        
        // Check if player is already on another team
        if self.is_player_on_team(data.player_id, team.tournament_id).await? {
            return Err(PawnError::ValidationError(
        ...
        if self.is_board_occupied(data.team_id, data.board_number).await? {
            return Err(PawnError::ValidationError(
                format!("Board {} is already occupied on this team", data.board_number)
            ));
        }
        ...
        let membership = self.db.add_player_to_team(data).await.map_err(PawnError::from)?;
    ...
    #[instrument(skip(self))]
    pub async fn remove_player_from_team(&self, data: RemovePlayerFromTeam) -> Result<(), PawnError> {
        info!("Removing player {} from team {}", data.player_id, data.team_id);
        
        // Check if player is on the team
        if !self.is_player_on_specific_team(data.player_id, data.team_id).await? {
            return Err(PawnError::ValidationError(
        ...
        if self.has_active_lineups(data.team_id, data.player_id).await? {
            return Err(PawnError::ValidationError(
                "Cannot remove player with active lineups".to_string()
            ));
        }
        
        self.db.remove_player_from_team(data).await.map_err(PawnError::from)?;
    ...
    #[instrument(skip(self))]
    pub async fn get_team_memberships(&self, team_id: i32) -> Result<Vec<TeamMembership>, PawnError> {
        self.db.get_team_memberships(team_id).await.map_err(PawnError::from)
    }

    /// Get team membership statistics
    #[instrument(skip(self))]
    pub async fn get_team_membership_statistics(&self, tournament_id: i32) -> Result<TeamMembershipStatistics, PawnError> {
        let memberships = self.db.get_all_team_memberships(tournament_id).await.map_err(PawnError::from)?;
        ...
        for team in teams {
            ...
            let active_count = team_memberships.iter().filter(|m| m.status == "active").count();
    ...
    pub async fn create_team_match(&self, data: CreateTeamMatch) -> Result<TeamMatch, PawnError> {
        info!("Creating team match between teams {} and {}", data.team_a_id, data.team_b_id);
        
        // Validate teams exist and are in the same tournament
        let team_a = self.db.get_team_by_id(data.team_a_id).await.map_err(PawnError::from)?;
        let team_b = self.db.get_team_by_id(data.team_b_id).await.map_err(PawnError::from)?;
        ...
        if self.match_exists(data.team_a_id, data.team_b_id, data.round_number).await? {
            return Err(PawnError::ValidationError(
                "Match already exists between these teams in this round".to_string()
            ));
        }
        
        let team_match = self.db.create_team_match(data).await.map_err(PawnError::from)?;
    ...
    pub async fn update_team_match(&self, data: UpdateTeamMatch) -> Result<TeamMatch, PawnError> {
        ...
        let existing_match = self.db.get_team_match_by_id(data.id).await.map_err(PawnError::from)?;
        
        // Validate status transition
        if let Some(ref new_status) = data.status {
            if !self.is_valid_status_transition(&existing_match.status, new_status) {
                return Err(PawnError::ValidationError(
                    format!("Invalid status transition from {} to {}", existing_match.status, new_status)
                ));
            }
        }
        ...
        if let (Some(a_match), Some(b_match), Some(a_board), Some(b_board)) = (
            data.team_a_match_points, data.team_b_match_points, 
            data.team_a_board_points, data.team_b_board_points
        ) {
            if (a_board + b_board) < (a_match + b_match) {
                warn!("Board points ({}) less than match points ({})", a_board + b_board, a_match + b_match);
            }
        }
        
        let team_match = self.db.update_team_match(data).await.map_err(PawnError::from)?;
    ...
    #[instrument(skip(self))]
    pub async fn get_team_matches(&self, tournament_id: i32, round_number: Option<i32>) -> Result<Vec<TeamMatch>, PawnError> {
        self.db.get_team_matches(tournament_id, round_number).await.map_err(PawnError::from)
    }

    /// Get team match statistics
    #[instrument(skip(self))]
    pub async fn get_team_match_statistics(&self, tournament_id: i32) -> Result<TeamMatchStatistics, PawnError> {
        ...
        let in_progress_matches = matches.iter().filter(|m| m.status == "in_progress").count() as i32;
    ...
    #[instrument(skip(self))]
    pub async fn create_team_lineup(&self, data: CreateTeamLineup) -> Result<TeamLineup, PawnError> {
        info!("Creating team lineup for team {} round {}", data.team_id, data.round_number);
        ...
        if self.lineup_exists(data.team_id, data.round_number, data.board_number).await? {
            return Err(PawnError::ValidationError(
                format!("Lineup already exists for board {} in round {}", data.board_number, data.round_number)
            ));
        }
        ...
        if data.is_substitute {
            if let Some(substituted_player_id) = data.substituted_player_id {
                if !self.is_player_on_specific_team(substituted_player_id, data.team_id).await? {
                    return Err(PawnError::ValidationError(
        ...
        let lineup = self.db.create_team_lineup(data).await.map_err(PawnError::from)?;
    ...
    #[instrument(skip(self))]
    pub async fn get_team_lineups(&self, team_id: i32, round_number: i32) -> Result<Vec<TeamLineup>, PawnError> {
        self.db.get_team_lineups(team_id, round_number).await.map_err(PawnError::from)
    }
    ...
    #[instrument(skip(self))]
    pub async fn create_team_tournament_settings(&self, data: CreateTeamTournamentSettings) -> Result<TeamTournamentSettings, PawnError> {
        info!("Creating team tournament settings for tournament {}", data.tournament_id);
        ...
        let settings = self.db.create_team_tournament_settings(data).await.map_err(PawnError::from)?;
    ...
    #[instrument(skip(self))]
    pub async fn update_team_tournament_settings(&self, data: UpdateTeamTournamentSettings) -> Result<TeamTournamentSettings, PawnError> {
        info!("Updating team tournament settings for tournament {}", data.tournament_id);
        
        // Validate at least one field is being updated
        if data.team_size.is_none() && data.max_teams.is_none() && data.match_scoring_system.is_none() {
            return Err(PawnError::ValidationError(
                "No fields to update".to_string()
            ));
        }
        
        let settings = self.db.update_team_tournament_settings(data).await.map_err(PawnError::from)?;
    ...
    #[instrument(skip(self))]
    pub async fn get_team_tournament_settings(&self, tournament_id: i32) -> Result<TeamTournamentSettings, PawnError> {
        self.db.get_team_tournament_settings(tournament_id).await.map_err(PawnError::from)
    }
    ...
    #[instrument(skip(self))]
    pub async fn get_team_statistics(&self, tournament_id: i32) -> Result<TeamStatistics, PawnError> {
        let teams = self.get_teams_by_tournament(tournament_id).await?;
    ...
    fn validate_tournament_settings(&self, data: &CreateTeamTournamentSettings) -> Result<(), PawnError> {
        ...
        if !["match_points", "board_points", "olympic_points", "custom"].contains(&data.match_scoring_system.as_str()) {
            return Err(PawnError::ValidationError(
    ...
    async fn is_team_name_taken(&self, tournament_id: i32, name: &str) -> Result<bool, PawnError> {
        let teams = self.get_teams_by_tournament(tournament_id).await?;
        Ok(teams.iter().any(|t| t.name.to_lowercase() == name.to_lowercase()))
    }

    /// Validate tournament supports teams
    async fn validate_tournament_for_teams(&self, tournament_id: i32) -> Result<(), PawnError> {
        let tournament = self.db.get_tournament_by_id(tournament_id).await.map_err(PawnError::from)?;
    ...
    async fn is_player_on_team(&self, player_id: i32, tournament_id: i32) -> Result<bool, PawnError> {
        let teams = self.get_teams_by_tournament(tournament_id).await?;
    ...
    async fn is_player_on_specific_team(&self, player_id: i32, team_id: i32) -> Result<bool, PawnError> {
    ...
    async fn is_board_occupied(&self, team_id: i32, board_number: i32) -> Result<bool, PawnError> {
        let memberships = self.get_team_memberships(team_id).await?;
        Ok(memberships.iter().any(|m| m.board_number == board_number && m.status == "active"))
    }
    ...
    async fn match_exists(&self, team_a_id: i32, team_b_id: i32, round_number: i32) -> Result<bool, PawnError> {
        let team_a = self.get_team_by_id(team_a_id).await?;
        let matches = self.get_team_matches(team_a.tournament_id, Some(round_number)).await?;
    ...
    async fn lineup_exists(&self, team_id: i32, round_number: i32, board_number: i32) -> Result<bool, PawnError> {
    ...
    pub async fn get_team_match_by_id(&self, match_id: i32) -> Result<TeamMatch, PawnError> {
        self.db.get_team_match_by_id(match_id).await.map_err(PawnError::from)
    }

    /// Get all team memberships for a tournament
    pub async fn get_all_team_memberships(&self, tournament_id: i32) -> Result<Vec<TeamMembership>, PawnError> {
        self.db.get_all_team_memberships(tournament_id).await.map_err(PawnError::from)
    }

    /// Get team standings for a tournament
    pub async fn get_team_standings(&self, tournament_id: i32) -> Result<Vec<TeamStanding>, PawnError> {
    ...
    pub async fn validate_team_lineup(&self, team_id: i32, round_number: i32) -> Result<bool, PawnError> {
        let lineups = self.get_team_lineups(team_id, round_number).await?;
    ...
    pub async fn validate_team_board_order(&self, team_id: i32, round_number: i32) -> Result<bool, PawnError> {
        let lineups = self.get_team_lineups(team_id, round_number).await?;
...
mod tests {
    ...
    async fn test_validate_tournament_settings() {
        ...
        assert!(service.validate_tournament_settings(&valid_settings).is_ok());
        ...
        assert!(service.validate_tournament_settings(&invalid_settings).is_err());
    }
    ...
    async fn test_create_team_success() {
        ...
        mock_db.expect_get_tournament_by_id()
            .with(eq(1))
            .returning(|_| Ok(Tournament {
                id: 1,
                name: "Test Tournament".to_string(),
                location: "Test Location".to_string(),
                date: "2024-01-01".to_string(),
        ...
        mock_db.expect_search_teams()
            .returning(|_| Ok(vec![]));

        mock_db.expect_create_team()
    ...
    async fn test_add_player_to_team_success() {
        ...
        mock_db.expect_get_player_by_id()
            .with(eq(1))
            .returning(|_| Ok(Player {
                id: 1,
                tournament_id: 1,
        ...
        mock_db.expect_add_player_to_team()
            .returning(|_| Ok(TeamMembership {
                id: 1,
                team_id: 1,
                player_id: 1,
                board_number: 1,
    ...
    async fn test_add_player_to_team_player_already_on_team() {
        ...
        mock_db.expect_get_all_team_memberships()
            .with(eq(1))
            .returning(|_| Ok(vec![TeamMembership {
                id: 1,
                team_id: 1,
                player_id: 1,
                board_number: 1,
    ...
    async fn test_remove_player_from_team_success() {
        ...
        mock_db.expect_get_team_lineups()
            .returning(|_, _| Ok(vec![]));

        mock_db.expect_remove_player_from_team()
            .returning(|_| Ok(()));
    ...
    async fn test_create_team_match_success() {
        ...
        mock_db.expect_create_team_match()
            .returning(|_| Ok(TeamMatch {
                id: 1,
                tournament_id: 1,
                round_number: 1,
                team_a_id: 1,
    ...
    async fn test_create_team_tournament_settings_success() {
        ...
        mock_db.expect_create_team_tournament_settings()
            .returning(|_| Ok(TeamTournamentSettings {
                id: 1,
                tournament_id: 1,
                team_size: 4,
                max_teams: Some(8),
