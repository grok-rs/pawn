...
impl<D: Db> ExportService<D> {
    ...
    async fn export_to_pdf(&self, data: &ExportData, file_path: &Path, request: &ExportRequest) -> Result<u64, PawnError> {
        ...
        // Tournament title
        current_layer.use_text(&format!("{}", data.tournament.name), 18.0, Mm(20.0), y_pos, &font_bold);
        y_pos -= Mm(10.0);
        
        // Tournament details
        current_layer.use_text(&format!("Location: {}", data.tournament.location), 12.0, Mm(20.0), y_pos, &font);
        y_pos -= Mm(6.0);
        current_layer.use_text(&format!("Date: {}", data.tournament.date), 12.0, Mm(20.0), y_pos, &font);
        y_pos -= Mm(6.0);
        current_layer.use_text(&format!("Players: {}", data.tournament.player_count), 12.0, Mm(20.0), y_pos, &font);
        y_pos -= Mm(6.0);
        current_layer.use_text(&format!("Rounds: {}/{}", data.tournament.rounds_played, data.tournament.total_rounds), 12.0, Mm(20.0), y_pos, &font);
        y_pos -= Mm(12.0);
        ...
        if let Some(ref standings) = data.standings {
            ...
            for (i, standing) in standings.standings.iter().enumerate() {
                if i >= 50 { // Limit to avoid page overflow
                    break;
                }
                ...
                current_layer.use_text(&format!("{}", standing.player.rating.unwrap_or(0)), 10.0, Mm(100.0), y_pos, &font);
                current_layer.use_text(&format!("{}", standing.points), 10.0, Mm(130.0), y_pos, &font);
                current_layer.use_text(&format!("{}-{}-{}", standing.wins, standing.draws, standing.losses), 10.0, Mm(160.0), y_pos, &font);
                y_pos -= Mm(5.0);
        ...
        // Add footer
        current_layer.use_text(&format!("Generated by Pawn Tournament Manager on {}", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC")), 8.0, Mm(20.0), Mm(10.0), &font);
    ...
    async fn export_to_xlsx(&self, data: &ExportData, file_path: &Path) -> Result<u64, PawnError> {
        ...
        info_sheet.write_string(4, 1, &format!("{}/{}", data.tournament.rounds_played, data.tournament.total_rounds))?;
        ...
        if let Some(ref standings) = data.standings {
            ...
            if let Some(first_standing) = standings.standings.first() {
                for tiebreak in &first_standing.tiebreak_scores {
                    standings_sheet.write_string_with_format(0, col, tiebreak.tiebreak_type.display_name(), &header_format)?;
                    col += 1;
                }
            }
            ...
            for (i, standing) in standings.standings.iter().enumerate() {
                ...
                standings_sheet.write_number_with_format(row as u32, 0, standing.rank as f64, &data_format)?;
                standings_sheet.write_string_with_format(row as u32, 1, &standing.player.name, &data_format)?;
                standings_sheet.write_number_with_format(row as u32, 2, standing.player.rating.unwrap_or(0) as f64, &data_format)?;
                standings_sheet.write_number_with_format(row as u32, 3, standing.points, &data_format)?;
                standings_sheet.write_number_with_format(row as u32, 4, standing.games_played as f64, &data_format)?;
                standings_sheet.write_number_with_format(row as u32, 5, standing.wins as f64, &data_format)?;
                standings_sheet.write_number_with_format(row as u32, 6, standing.draws as f64, &data_format)?;
                standings_sheet.write_number_with_format(row as u32, 7, standing.losses as f64, &data_format)?;
                
                // Add tiebreak scores
                let mut col = 8;
                for tiebreak in &standing.tiebreak_scores {
                    standings_sheet.write_string_with_format(row as u32, col, &tiebreak.display_value, &data_format)?;
                    col += 1;
                }
            }
        ...
        for (i, player) in data.players.iter().enumerate() {
            ...
            players_sheet.write_number_with_format(row as u32, 1, player.rating.unwrap_or(0) as f64, &data_format)?;
            players_sheet.write_string_with_format(row as u32, 2, player.country_code.as_deref().unwrap_or(""), &data_format)?;
            players_sheet.write_string_with_format(row as u32, 3, player.title.as_deref().unwrap_or(""), &data_format)?;
