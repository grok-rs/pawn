...
use crate::pawn::{
    ...
    state::PawnState,
    domain::{
        model::{KnockoutBracket, BracketPosition, Pairing},
...
pub async fn initialize_knockout_tournament(
    ...
    let first_round_positions = KnockoutService::generate_first_round_positions(
        created_bracket.id,
        players,
    );
...
pub async fn get_bracket_positions_by_round(
    ...
    let db = &*state.db;
    let positions = db.get_bracket_positions_by_round(bracket_id, round_number).await?;
    Ok(positions)
}
...
pub async fn advance_knockout_winners(
    ...
    let next_round_positions = KnockoutService::advance_winners(
        bracket_id,
        round_number,
        &winner_results,
    );
