...
async fn test_basic_crud_operations() {
    ...
    assert!(tournament_insert.is_ok(), "Should be able to insert tournament");
...
async fn test_migration_idempotency() {
    ...
    assert!(first_migration.is_ok(), "First migration failed: {:?}", first_migration);
    
    // Apply migrations second time (should be idempotent)
    let second_migration = sqlx::migrate!("./migrations").run(&pool).await;
    assert!(second_migration.is_ok(), "Second migration failed: {:?}", second_migration);
    ...
    assert!(migration_records.len() >= 10, "Expected at least 10 migration records");
...
async fn test_foreign_key_constraints() {
    ...
    let insert_result = sqlx::query("INSERT INTO players (tournament_id, name) VALUES (99999, 'Test Player')")
        .execute(&test_db.pool)
        .await;
...
async fn test_large_dataset_performance() {
    ...
    let search_results = sqlx::query("SELECT * FROM players WHERE tournament_id = ? AND rating BETWEEN ? AND ?")
        .bind(tournament_id)
        .bind(1500)
        .bind(1700)
        .fetch_all(&test_db.pool)
    ...
    assert!(insertion_time.as_millis() < 5000, "Player insertion took too long: {}ms", insertion_time.as_millis());
    assert!(search_time.as_millis() < 100, "Player search took too long: {}ms", search_time.as_millis());
