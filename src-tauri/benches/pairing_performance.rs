        ...
        group.bench_with_input(
            ...
            |b, &count| {
                b.to_async(&rt).iter(|| async {
                    let benchmark_db = BenchmarkDb::new()
                        .await
                    ...
                    // Create players
                    let players = create_test_players(tournament.id, count, &player_service)
                        .await
                        .expect("Failed to create players");

                    // Benchmark pairing generation
                    let pairing_engine = SwissPairingEngine::new();
                    let swiss_players: Vec<SwissPlayer> = players
                        .into_iter()
                        .map(|p| SwissPlayer {
                            id: p.id,
                            name: p.name,
            ...
            |b, &count| {
                b.to_async(&rt).iter(|| async {
                    ...
                    for round in 1..=total_rounds {
                        let round_robin_players: Vec<RoundRobinPlayer> = players
                            .iter()
                            .map(|p| RoundRobinPlayer {
                                id: p.id,
                                name: p.name.clone(),
                                rating: p.rating.unwrap_or(1500),
                                tournament_id: p.tournament_id,
                        ...
                        black_box(
                            pairing_engine.generate_pairings(round_robin_players, round as i32),
                        )
        ...
        group.bench_with_input(
            ...
            |b, &count| {
                b.to_async(&rt).iter(|| async {
                    ...
                    // Create players
                    let _players = create_test_players(tournament.id, count, &player_service)
                        .await
                        .expect("Failed to create players");
                    ...
                    black_box(
                        knockout_service
                            .initialize_knockout_tournament(
                                tournament.id,
                ...
                b.to_async(&rt).iter_batched(
                    || {
                        ...
                        rt.block_on(async {
                            ...
                            let tournament_service =
                                TournamentService::new(benchmark_db.db.clone());
                            let player_service = PlayerService::new(benchmark_db.db.clone());
                            ...
                            create_test_players(tournament.id, count, &player_service)
                                .await
                                .expect("Failed to create players");
...
fn bench_memory_usage(c: &mut Criterion) {
    ...
    group.bench_function("tournament_lifecycle", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            // Create players
            let players = create_test_players(tournament.id, 100, &player_service)
                .await
                .expect("Failed to create players");
            ...
            black_box(
                tournament_service
                    .get_tournament_standings(tournament.id)
                    .await,
            )
            .expect("Failed to calculate standings");
        });
